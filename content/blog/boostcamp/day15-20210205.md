...
이를 구현한 것이 바로 **VAE**고 학습데이터 외에 새로운 데이터를 만들기 때문에 생성모델이라고 볼 수 있다.

하지만 **AE**는 <u>단순히 입력을 잠재공간으로 매핑한 후 디코더로 복원하는 것</u>이므로 생성모델이라고 볼 수 없다. AE에 무작위 데이터를 넣으면 이상한 출력이 나오는 것을 볼 수 있다.
