...
- $X$와 $Y$를 내적한다고 할 때, **$X$번째 행의 개수 == $Y$의 행의 개수**여야 한다.
...
`조건 1`을 만족하지 않는 즉, 정사각행렬이 아닌 다른 모양의 행렬에서는 역행렬 대신에 **유사 역행렬(Pseudo Inverse Matrix)**를 정의할 수 있다. 참고로 유사 역행렬을 구해서 대입해보면 값이 틀린 것을 볼 수 있다. 애초에 역행렬을 만들 수 없는 행렬이기 때문에 **이에 근사한 역행렬**을 만든 것이 유사 역행렬이다.
...
beta = np.linalg.pinv(X) @ y    # 회귀 계수
y_predict = X @ beta            # 회귀식을 예측한 y
```
