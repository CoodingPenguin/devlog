...
<p class="content__title">왜 표본분산을 구할 때 <span class="katex">N</span>이 아니라 <span class="katex">N-1</span>로 나눌까?</p>
<p class="content__content">불편추정량(unbiased estimator)를 구하기 위해서이다. 여기서 불편추정량이란 편의가 없는 추정량을 말하며, 편의는 모수와 추정치의 차이를 말한다. <span class="katex">N</span>이 아닌 <span class="katex">N-1</span>로 나누었을 때가 모수를 더 정확히 추정하기 때문에 <span class="katex">N-1</span>로 나누는 것이다.</p>
</div>
</div>
