...
하지만 <u>한 연산자의 개수가 2개 이상인 경우 중복이 존재</u>한다. 중복 제거를 위해 *set()*을 씌워 없애준다. 이렇게 데이터가 다 준비되면 다음과 같은 과정으로 연산자의 경우의 수인 `operatersList`를 순회하여 **최소값과 최대값**을 구하였다.
...
[skatn3님의 풀이](https://www.acmicpc.net/source/22309161)를 참고하였으며 **DFS를 이용한 풀이**이다. 앞서 다른 문제에서 설명하다싶이 DFS는 무엇을 기준으로 언제 가지치기를 하고 중단하는지 중요하다. 그래서 `분기점`과 `리프노드의 조건`은 다음과 같다.
...
**DFS**의 방식을 정리하자면 리프노드에 닿을 때까지 `재귀`를 통해서 계속 깊어지며 리프노드에 도착하면 그 때의 결과 값을 사용자의 의도에 따라 처리하면 된다.
