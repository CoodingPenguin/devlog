...
일단 알아둬야할 것! **백준에서는 `numpy`와 같은 라이브러리를 지원하지 않는다!** 오직 표준 라이브러리만 지원한다. 이걸 모르고 `numpy`로 구현을 해서 *런타임 에러*가 났는데 애꿎은 *인덱스 에러*만 찾아보고 있었다..ㅠㅠ

문제에서 가능한 체스판은 맨 왼쪽 위 칸이 <u>흰색인 경우</u>와 <u>검은색인 경우</u>이다. 그래서 두 가지 경우를 모두 검사를 해야한다. 이를 고려하여 알고리즘을 짜면 다음과 같다.
